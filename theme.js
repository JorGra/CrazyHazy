(function hazy() {
  ...
  function valueSet() {
    ...
    const listAnimationStrenghtValue = parseInt(localStorage.getItem("AnimationStrenghtListItems"));
    ...
    if (!isNaN(listAnimationStrenghtValue)) {
      document.documentElement.style.setProperty("--animation-strength-list-items", `${listAnimationStrenghtValue}`);
    } else {
...
;
  ...
  const homeEdit = new Spicetify.Topbar.Button("Hazy Settings", "edit", () => {
    ...
    function setValue(blur_am, cont, satu, bright, animStrength, desc) {
      ...
      let animStrength_val = localStorage.getItem(animStrength);
    ...
    if (localStorage.getItem(bright) === null) {
      ...
      <input class="slider" id="animStrength-input" type="range" min="0" max="150" step="1" value="${animStrength_val}">
      <div class="slider-value">
    ...
    valueRow.querySelector("#animStrength-value").addEventListener("input", function() {
      ...
      if (content.length > 3) {
        ...
        valueRow.querySelector("#animStrength-input").value = number;
      });
    ...
    setValue("blurAmount", "contAmount", "satuAmount", "brightAmount", "AnimationStrenghtListItems"," ")
